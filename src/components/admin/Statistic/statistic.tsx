...
const CustomTooltip: React.FC<TooltipProps<number, string>> = ({ active, payload, label }) => {
  ...
  return (
    <Paper sx={{ p: 1 }}>
      ...
      <Typography variant="body2">
        {completedValue.toLocaleString()} (completed: {completedCount}) –{' '}
        {canceledValue.toLocaleString()} (canceled: {canceledCount})
      </Typography>
    </Paper>
  );
};
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const productsSold = completedOrders
    ...
    orderService.getOrderHistory(accessToken, 1, 1000)
      .then(res => {
        const all = res.data.data.filter(o =>
          dayjs(o.createdAt).format('YYYY-MM-DD') === data.date
        );
